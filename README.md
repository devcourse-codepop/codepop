...
- UI
    - 프로필, 프로필 수정, 팔로잉 & 팔로워 리스트, 메시지
- 기능
    - 프로필, 프로필 수정, 팔로잉 & 팔로워 리스트 표시, 팔로우 & 팔로우 취소 기능
...
초기에 다크모드 기능을 구현하는데 집중하면서, 관련 로직과 타입을 각 컴포넌트 내부에 중복 작성하는 방식으로 처리하였습니다. 이후 페이지 수가 늘어나면서 중복된 코드가 점점 많아지고 있다는 문제를 인식하게 되었습ㄴ디ㅏ. 이를 개선하기 위해 각 컴포넌트에 반복 작성되던 다크모드 판별 조건을 공통 유틸 함수로 분리하고, 관련 타입 정의도 별도 파일로 이동하여 전체 구조를 정리하였습니다.
